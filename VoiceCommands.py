...
def get_events(phrase):
    ...
    while True:
        ...
        if "today" in event_phrase:
            ...
            weekday_num = -1
            for weekday in list(weekday_to_num.keys()):
                if weekday.lower() in event_phrase.lower():
    ...
    for num in num_list:
        ...
        if str(start_day) == str(start_time.day):
            ...
            print(convert_time(event_dict[num]['summary'] + " that goes from " + start[11:13] + ":" + start[14:16] + " to " + end[11:13] + ":" + end[14:16] + ". "))
            say(convert_time(event_dict[num]['summary'] + " that goes from " + start[11:13] + ":" + start[14:16] + " to " + end[11:13] + ":" + end[14:16] + ". "))
...
def set_event(phrase):
    ...
    while True:
        ...
        if "today" in event_phrase:
            ...
            weekday_num = -1
            for weekday in list(weekday_to_num.keys()):
                print("Weekday: " + str(weekday.lower()))
                if weekday.lower() in event_phrase.lower():
                    weekday_num = weekday_to_num[weekday]
            if weekday_num != -1:
                next_week = False
...
def cancel_command(phrase):
    print("Canceled")

def exit_program(phrase):
    os._exit(1)

commands = {"say": repeat, "weather": getWeather, "temp": getWeather, "remind": set_reminder, "timer": set_reminder, "alarm":set_alarm, "left": get_time_left, "what time": get_time, "date": get_date, "calendar":set_event, "schedule":get_events, "cancel": cancel_command, "exit": exit_program}
...
def convert_time(time_phrase):
    t_phrase = tokenize(time_phrase)
    for i in range(len(t_phrase)):
        word = t_phrase[i]
        if ':' in word:
            hour = int(word[0:word.index(':')])
            minute = word[word.index(':') + 1:]
            ...
            if hour > 12:
                hour = hour - 12
                new_time = str(hour) + " " + minute + " p.m."
            elif hour == 0:
                hour = 12
                new_time = str(hour) + " " + minute + " a.m."
            else:
                new_time = str(hour) + " " + minute + " a.m."
            t_phrase[i] = new_time
    new_phrase = ''
